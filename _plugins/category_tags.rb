    ...
    def new_tag(tag, posts)
      self.class.types.each { |type|
        if layout = site.config["tag_#{type}_layout"]
          ...
          # tag_dir — modified by Chris
          #
          #   I look at the first post tagged with this tag and make an assumption
          #   about the tag's category based on the post category. It's buggy in
          #   cases where a tag is shared between both blogs, but I'm the only one
          #   creating content so I will just be careful 😁
          tag_dir = data['posts'][0]['category'] + '/' + site.config["tag_#{type}_dir"]
